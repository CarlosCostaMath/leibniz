(function () {
  ...
  function buildSnippet(post, query, tokens) {
    ...
    const preparedTokens = Array.isArray(tokens) ? tokens : getTokens(query);
    const normalisedQuery = removeDiacritics((query || '').trim()).toLowerCase();

    if (!preparedTokens.length) {
      return escapeHtml(excerpt);
    }

    const normalisedExcerpt = removeDiacritics(excerpt).toLowerCase();
    const excerptHasAllTokens = preparedTokens.every((token) =>
      normalisedExcerpt.includes(token)
    );
    const excerptHasPhrase = Boolean(normalisedQuery) && normalisedExcerpt.includes(normalisedQuery);

    if (excerpt && (excerptHasPhrase || excerptHasAllTokens)) {
      return emphasiseTerms(excerpt, '', preparedTokens);
    }
    ...
    if (normalisedQuery && normalisedContent.includes(normalisedQuery)) {
      matchIndex = normalisedContent.indexOf(normalisedQuery);
